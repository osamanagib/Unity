...
{
    public class RealWorldTerrainArcGISElevationGenerator : RealWorldTerrainElevationGenerator
    {
        private static string arcGisFolder;
        ...
        private RealWorldTerrainArcGISElevationGenerator(double x1, double y1, double x2, double y2, double tx1, double ty1, double tx2, double ty2)
        {
            this.tx1 = tx1;
            this.ty1 = ty1;
            this.tx2 = tx2;
            ...
            filename = Path.Combine(
                ...
                String.Format(RealWorldTerrainCultureInfo.numberFormat, 
                    "arcgis_{0}x{1}x{2}x{3}x{4}.rwt", x1, y1, x2, y2, mapSize));
        ...
        private void Download()
        {
            ...
            for (int x = 0; x < s; x++)
            {
                ...
                for (int y = 0; y < s; y++)
                {
                    ...
                    string partFilename = Path.Combine(arcGisFolder, 
                        string.Format(RealWorldTerrainCultureInfo.numberFormat, 
                            "arcgis_{0}x{1}x{2}x{3}x{4}.json", sx, sy, ex, ey, mapSize));

                    if (!File.Exists(partFilename))
                    {
                        string partURL = "http://sampleserver4.arcgisonline.com/ArcGIS/rest/services/Elevation/ESRI_Elevation_World/MapServer/exts/ElevationsSOE/ElevationLayers/1/GetElevationData?f=json&Extent={%22spatialReference%22:{%22wkid%22:4326},%22ymin%22:" 
                                         + ey.ToString(RealWorldTerrainCultureInfo.numberFormat) + ",%22ymax%22:" 
                                         + sy.ToString(RealWorldTerrainCultureInfo.numberFormat) + ",%22xmin%22:" 
                                         + sx.ToString(RealWorldTerrainCultureInfo.numberFormat) + ",%22xmax%22:" 
                                         + ex.ToString(RealWorldTerrainCultureInfo.numberFormat) + "}&Rows=32&Columns=32";
        ...
        public static void GetArcGISElevationRange(out double minEl, out double maxEl)
        {
            minEl = double.MaxValue;
            maxEl = double.MinValue;

            foreach (RealWorldTerrainArcGISElevationGenerator elevation in elevations)
            {
        ...
        public static void Init()
        {
            ...
            arcGisFolder = Path.Combine(RealWorldTerrainEditorUtils.heightmapCacheFolder, "ArcGISParts");
            if (!Directory.Exists(arcGisFolder)) Directory.CreateDirectory(arcGisFolder);
            ...
            elevations.Add(new RealWorldTerrainArcGISElevationGenerator(cx1, cy1, cx2, cy2, stx, sty, etx, ety));
        }
        ...
        public static bool Load()
        {
            ...
            {
                RealWorldTerrainArcGISElevationGenerator elevation = (RealWorldTerrainArcGISElevationGenerator)elevations[i];
