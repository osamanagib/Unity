...
namespace AQUAS
{
    public class AQUAS_BubbleBehaviour : MonoBehaviour
    {
        ...
        void Start()
        {
            maxSmallBubbleCount = (int)Random.Range(1, 2);
            smallBubbleCount = 0;
        ...
        void SmallBubbleSpawner()
        {
            if (smallBubbleCount <= maxSmallBubbleCount)
            {
                ...
                Instantiate(smallBubble, new Vector3(transform.position.x + Random.Range(-0.1f, 0.1f), transform.position.y - Random.Range(0.01f, 1), transform.position.z + Random.Range(-0.1f, 0.1f)), Quaternion.identity).transform.SetParent(this.transform.parent);
