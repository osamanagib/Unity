...
namespace InfinityCode.HugeTexture.Editors
{
    ...
    {
        private const string packageID = "Huge Texture";
        private const string assetPrefix = "HugeTexture.";
        ...
        public static void CheckNewVersionAvailable()
        {
            ...
            client.UploadDataCompleted += delegate (object sender, UploadDataCompletedEventArgs response)
            {
                ...
                {
                    Debug.Log("Automatic check for Huge Texture updates: Bad response.");
                    return;
                }
        ...
        {
            double v1 = GetDoubleVersion(HugeImporter.version);
        ...
        private void GetUpdateList(string updateKey)
        {
            ...
            try
            {
                response = client.UploadString("http://infinity-code.com/products_update/checkupdates.php",
                    "k=" + UnityWebRequest.EscapeURL(updateKey) + "&v=" + HugeImporter.version + "&c=" + (int)channel);
            }
        ...
        private void OnEnable()
        {
            ...
            Texture2D helpTexture = EditorUtils.LoadAsset<Texture2D>("Textures/HelpIcon.png");
            helpContent = new GUIContent(helpTexture, "You can find out your Invoice Number in the email confirming the purchase, or page the user in Unity Asset Store.\nClick to go to the Unity Asset Store.");
        }
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("Current version: " + HugeImporter.version);
        ...
        [MenuItem(HugeTextureImporter.MenuPath + "Check Updates", false, 123)]
        public static void OpenWindow()
        {
            GetWindow<Updater>(false, "Huge Texture Updater", true);
        }
